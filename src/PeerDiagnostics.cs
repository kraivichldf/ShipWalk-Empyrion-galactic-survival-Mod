...
namespace ShipWalk
{
    ...
    // Run on both clients: the pilot's trace then describes the passenger seen
    // by the pilot, while the passenger records their own local movement.
    internal sealed class PeerDiagnostics
    {
        ...
        {
            try { CapturePlayers(); }
            catch (Exception error) { log.Info("MP peers unavailable; reason=" + error.GetType().Name); }
        }
        private void CapturePlayers()
        {
            IPlayfield playfield = api.ClientPlayfield;
            if (playfield == null) { log.Info("MP peers; no active playfield."); return; }
            ...
            log.Info("MP peers; observer=" + (observer?.Id ?? -1)
            ...
            foreach (IPlayer player in players)
            {
                try
                {
                    ...
                    {
                        log.Info("MP observed; observer=" + (observer?.Id ?? -1) + "; actor=" + player.Id
                    ...
                    log.Info("MP observed; observer=" + (observer?.Id ?? -1)
                ...
                catch (Exception error)
                {
                    ...
                    log.Info("MP observed unavailable; actor=" + player.Id + "; reason=" + error.GetType().Name);
                }
            }
        }
    }
